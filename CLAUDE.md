...
| AP 7 | Studio (eigene Videos verwalten)                                                                                                    | 3 PT                                                                                                              | teilweise umgesetzt (Liste eigener Videos über `/api/me/videos` inkl. Cursor/Load-more, Löschen über `DELETE /api/videos/{id}` mit `ConfirmDialog`-Bestätigung, Sichtbarkeits-Toggle "Make public"/"Make private" über `PATCH /api/videos/{id}`; Metadaten-Bearbeiten weiterhin offen, siehe §12)                                                                                                                                                                                                                                             |
...
- [x] Katalog-Feed paginiert serverseitig über `cursor`/`nextCursor`, nicht über Seitenzahlen — AP 4 hat die Overview-Seite auf ein Cursor-/Load-more-Pattern umgestellt (weicht vom Mockup ab). Die in AP 1 gebaute Seitenzahlen-Pagination-Komponente in `shared/pagination/` wird seit 2026-09-13 von der Suche genutzt (siehe nächster Punkt).
- [x] Titelsuche (2026-09-13): Backend `GET /api/search/videos?q=&sort=relevance|newest&page=&includeAgeRestricted=` liefert `PageResponseVideoSummaryDto` (`items`, `page` ab 1, `size` = 50 fest, `totalItems`, `totalPages`). Bewusst Seitenzahlen statt Cursor — dokumentierte Ausnahme in §3.2 der Backend-CLAUDE.md (Relevanz-Sortierung hat keinen stabilen Keyset). Suche nur im Titel, fehlertolerant über `pg_trgm` (Wortteile + Tippfehler), nur `PUBLIC`/`READY`, `q` muss getrimmt 2–100 Zeichen lang sein. Frontend: Route `/catalog/search` (`features/catalog/search/`), die URL (`q`, `sort`, `page`) ist einzige Quelle der Wahrheit, Daten über `httpResource`, Ergebnisse mit `VideoRow`, Seitenwechsel scrollt über `ViewportScroller` nach oben. Suchfeld als dumme Komponente `shared/search-box/` (Signal Forms, emittiert getrimmten Query, Navigation macht der Host). Header: ab `md` Suchfeld inline, darunter nur Lupen-Icon-Link zur Suchseite (die ihr eigenes Feld nur unterhalb `md` zeigt) — wegen der Header-Breite auf 375–390px, siehe Eintrag 2026-09-01. `schema.ts` wurde von Hand um Pfad/Operation/Schema ergänzt, weil das Backend beim Implementieren nicht lief — beim nächsten `npm run generate:api` gegen das echte Backend verifizieren. Noch offen: `e2e/mobile-layout.spec.ts` gegen den eingeloggten **Admin**-Header laufen lassen (Icon-Link kostet ~40px zusätzlich).
- [x] Asynchrones Löschen (2026-09-14): `DELETE /api/videos/{id}` antwortet jetzt mit `202 Accepted` statt `204` — das Backend blendet das Video sofort aus (neuer Status `DELETING`, auch für den Owner unsichtbar) und löscht Storage + DB-Zeile per Job (Backend-CLAUDE.md §9.7). Grund: große Videos liefen beim synchronen Löschen ins 30-s-Timeout. Frontend-seitig keine Logik-Änderung nötig — Studio und Admin-Reports werten jeden 2xx als Erfolg und entfernen die Zeile bereits optimistisch. Neu möglich: `409`, solange ein Transcode oder Sichtbarkeitswechsel für das Video läuft (Detailtext kommt über den bestehenden `describeError`-Pfad). `DELETING` wurde von Hand in `schema.ts` und `VideoLifecycleStatus` ergänzt (taucht praktisch nur in `AdminReportDto.videoStatus` auf) — beim nächsten `npm run generate:api` verifizieren.
...
- [x] Studio — "Meine Videos" + Löschen (2026-08-30): Erster Teil von AP 7 umgesetzt. `GET /api/me/videos` (cursor-paginiert, `VideoDetailDto`) und `DELETE /api/videos/{id}` waren bereits im generierten Schema vorhanden, keine Backend-Änderung nötig. Bestätigt: der Video-Owner (nicht nur Admin) darf `DELETE /api/videos/{id}` für eigene Videos aufrufen — unabhängig vom offenen Punkt weiter oben zu Admin-Delete auf fremde Videos. Löschbestätigung lief zunächst 1:1 über die bestehende `ConfirmDialog`-Komponente aus dem Admin-Reports-Flow (Pflichtfeld "Reason"); auf Nachfrage noch am selben Tag revidiert (siehe nächster Punkt) zu einem echten Ja/Nein ohne Reason-Zwang. Metadaten-Bearbeiten ist bewusst nicht Teil dieses Nachtrags.
- [x] `ConfirmDialog` — Reason optional gemacht (2026-08-30): Neuer Input `requireReason` (Default `true`, erhält das bestehende Admin-Verhalten unverändert). Bei `false` entfällt Label/Textarea/Validierung komplett, `onConfirm()` emittiert sofort (leerer String). Studio nutzt `[requireReason]="false"` für die Delete-Bestätigung — jetzt ein reines Ja/Nein-Modal, kein Freitext-Zwang mehr für eine reine Owner-Aktion ohne Audit-Trail-Bedarf.
- [x] Sichtbarkeits-Toggle "Make public" / "Make private" (2026-08-30): In der Studio-Liste umgesetzt, `PATCH /api/videos/{id}` mit `UpdateVideoRequest{ visibility }` (`StudioApi.updateVisibility`). Bewusst **kein** Bestätigungsmodal — reversible, nicht-destruktive Aktion, anders als Löschen. Bei Erfolg wird das Visibility-Tag optimistisch aktualisiert; der `409 Conflict`-Fall (offener Report, `detail: "Video cannot be made public while a report is open"`) läuft über den bestehenden `isApiProblem`/`describeError`-Pfad und zeigt den Backend-Detailtext direkt inline an, keine Sonderbehandlung nötig. Die generierte Spec dokumentiert für `update` `200 OK` mit `VideoDetailDto`-Body statt der vom Backend-Kollegen genannten `202 Accepted` — da der optimistische UI-State ohnehin nicht vom Response-Body abhängt (nur vom Erfolg/Fehlschlag), ist das für das Frontend folgenlos. Migration synchron-vs-Job-basiert (§9.5) bleibt separates, größeres Thema.
...
- [x] Häufige unerwartete Logouts (2026-08-31): Der Access-Token liegt bewusst nur im Speicher (§8/CLAUDE.md des Backends, XSS-Schutz) — jeder Reload/neue Tab hängt deshalb komplett davon ab, dass `POST /api/auth/refresh` beim App-Start (`provideAppInitializer` → `AuthService.restoreSession()`) klappt. Live beobachtet: ein 403 (nicht 401 — also CSRF-Check-Fehler, kein abgelaufener/ungültiger Token) auf genau diesem Call, verursacht u. a. dadurch, dass der `refresh_token`-Cookie backend-seitig `SameSite=Strict` war und bei jeder Cross-Site-Navigation in die App hinein (externer Link, manche Tab-Restore-Fälle) gar nicht erst mitgeschickt wurde. Backend-Fix (video-service-Repo): Cookie auf `SameSite=Lax` gelockert (weiterhin POST-only + eigener CSRF-Token-Check, also nicht schwächer gegen CSRF). Frontend-Fix in `AuthService` (`core/auth/auth.ts`): proaktiver Refresh bei ~80 % der `expiresInSeconds`-TTL statt rein reaktiv erst nach einem 401 — macht die 15-Minuten-Grenze im Normalbetrieb unsichtbar. Zusätzlich behoben: `refreshAccessToken()` rief bei einem wirklich fehlgeschlagenen Refresh nirgends `clearSession()` auf (weder der reaktive Interceptor noch der neue proaktive Timer) — `isAuthenticated()` blieb fälschlich `true` für einen Token, der nie wieder funktioniert. Jetzt zentral in `refreshAccessToken()` selbst. Unit-Tests in `auth.spec.ts` (Vitest Fake-Timers) decken proaktiven Refresh und den Clear-on-failure-Fall ab.
...
- [x] Rebrand (2026-09-20): Markenname von "sol-stream" auf **"Video Platform"** umbenannt, Mark-Buchstabe von "S" auf "V" gewechselt. Geändert: `src/index.html` (`<title>`), `src/app/layout/header/header.html` und `src/app/features/auth/login/login.html` (Mark + Wordmark; der Mark ist jetzt `aria-hidden`, dafür trägt der Brand-Link im Header ein `aria-label="Video Platform – home"`, weil das Wordmark unterhalb `sm` ausgeblendet ist), `src/app/features/catalog/search/search.ts` (beide `$localize`-Seitentitel — neue Message-IDs, folgenlos solange nur `en` extrahiert wird, §4.4), `src/styles/_tokens.scss` (Kommentar). `public/favicon.ico` neu generiert (16/32/48px, "V" als Polygon mit spitzem Fuß auf der `--gradient-accent`-Kachel, `--color-on-accent` als Glyphenfarbe); erzeugt mit einem Wegwerf-Node-Skript (PNG-freie BMP-Einträge, keine neue Dependency), das bewusst **nicht** eingecheckt ist. Domain `sol-stream.space`, DNS/Hosting/CSP bleiben unangetastet — Infra-Thema aus AP 9.
- [x] Header-Overflow im eingeloggten Admin-Header behoben (2026-09-20, Folge des Rebrands): Gemessen (Chromium, Auth über gemockte `/api/auth/refresh` + `/api/me`, da kein Backend lief): der Admin-Header lief bei 480px schon **vor** dem Rebrand 31px über seine eigene Zeile, mit dem breiteren Wordmark "Video Platform" 43px; bei 375px 4px (wordmark-unabhängig, Ursache ist der Lupen-Icon-Link aus dem Suche-Nachtrag). Sichtbar war das nicht als Seiten-Scroll, weil `_reset.scss` seit 2026-09-01 `overflow-x: hidden` setzt — der Header-Inhalt wurde also still abgeschnitten. Fix in `header.scss`: die Spacing-Sprünge (`.header`-Padding, `.header__start`/`.header__nav`/`.header__end`-Gaps) laufen jetzt erst ab `md` statt ab `sm`, und die Nav-Gap unterhalb `md` ist `--space-2` statt `--space-3`. Das Wordmark bleibt ab `sm` sichtbar (kein Inhalt wird versteckt). Nachgemessen: 375/390/479/480/560/640/768/820/1024px jeweils 0px Overflow für USER **und** ADMIN. Damit ist der offene Punkt "mobile-layout gegen den Admin-Header" aus dem 2026-09-13-Eintrag erledigt.
- [x] Rest-Overflow im Admin-Header unterhalb 375px behoben (2026-09-20, Variante A): Ausgangslage war 11px Überlauf bei 360px und 51px bei 320px — wordmark-unabhängig, Alt-Bestand aus dem Suche-Nachtrag, wegen `overflow-x: hidden` als stilles Abschneiden statt als Scrollen sichtbar. Die Textlinks allein machten 214px aus (Browse 48, Admin 42, Upload 46, Studio 42, Sign out 36). Umgesetzt: Nav-Links und der Sign-out-Button klappen unterhalb `md` auf Icons zusammen (`header__link-icon` + `header__link-label` im Template, Umschaltung rein per CSS), der Name hängt jeweils am `aria-label` und bleibt damit für Screenreader in beiden Modi erhalten; Icons sind Inline-SVGs im Stil des bestehenden Lupen-Icons (Home, Shield, Upload-Pfeil, Play-Rechteck, Power), Touch-Target 28px wie beim Suche-Icon. Umschaltpunkt bewusst `md` (768px) statt `sm`: bei exakt 480px passen die Textlabels zwar rechnerisch, aber Wordmark und "Sign out" brechen zweizeilig um. Zusätzlich `flex: none` + `white-space: nowrap` auf allen Header-Controls (das Suche-Icon wurde vorher auf 19px statt 28px zusammengedrückt, "Sign out" brach bis 1024px zweizeilig um) und `white-space: nowrap` auf dem Wordmark. Nachgemessen (Chromium, gemockter Admin-Login): 320/360/375/390/480/600/767/768/800/900/1024/1280px jeweils 0px Overflow, einzeilige Controls. Playwright-Abdeckung in `mobile-layout.spec.ts`: Icon-only bei 375px inkl. erreichbarem Namen, Textlabel ab 768px (ausgeloggt, daher ohne Backend lauffähig).
- [x] `login()`-Helper in `e2e/mobile-layout.spec.ts` und `e2e/upload.spec.ts` repariert (2026-09-20, beim Verifizieren des Rebrands gefunden): Der Selector `button[type="submit"]` war seit dem Suchfeld-Nachtrag (2026-09-13) mehrdeutig, weil die `search-box` im Header einen eigenen Submit-Button rendert — Playwright brach mit "strict mode violation" ab, beide Suiten konnten sich nicht mehr einloggen. Jetzt auf `.auth__form button[type="submit"]` eingegrenzt. Vorbestehender Bruch, nicht vom Rebrand verursacht.
- [x] `e2e/mobile-layout.spec.ts` um 480px und 560px erweitert (2026-09-20): erst ab `sm` (480px) ist das Wordmark überhaupt sichtbar, also sind das die schmalsten Viewports, in denen der Markenname Overflow verursachen kann. Ausgeloggt laufen alle Fälle grün (Chromium + WebKit, 8/8); die eingeloggten Fälle brauchen den lokalen Backend auf `localhost:8080` und wurden in dieser Session nicht mitverifiziert (Backend lief nicht) — stattdessen per gemocktem Auth-Zustand gemessen, siehe Overflow-Eintrag oben.
- [x] Profilfoto (2026-09-19): Backend (video-service, CLAUDE.md §9.8) liefert `avatarUrl` auf `UserResponse`, `ownerAvatarUrl` auf `VideoDetailDto`, `PUT`/`DELETE /api/me/avatar` (multipart, Feld `file`), `GET /api/users/{username}` (`PublicUserResponse`) und `POST /api/admin/users/{username}/avatar/remove` (Reason Pflicht, Audit-Log). Das Backend schneidet serverseitig mittig quadratisch zu (256×256 JPEG), jeder Upload bekommt eine neue URL — kein Cache-Busting im Frontend nötig, CSP `img-src` deckt die Storage-Origin bereits ab. Frontend: `shared/avatar` nimmt jetzt `name` (Initialen berechnet die Komponente selbst), optional `src` und `size` (`sm`/`md`/`lg`), fällt bei fehlendem oder nicht ladbarem Bild auf die Initialen zurück. Der Header-Avatar ist ein Button, der `features/auth/profile-photo-dialog/` öffnet (per `@defer (on interaction)` eigener Lazy-Chunk, ~2,7 kB) — Vorschau über dieselbe Avatar-Komponente (`object-fit: cover` = exakt der Server-Zuschnitt), Upload/Entfernen über `ProfileApi`, Ergebnis landet über `AuthService.updateCurrentUser()` sofort im Header. Detailseite zeigt den Uploader-Avatar; Admins sehen dort "Remove profile photo" (bestehender `ConfirmDialog` mit Reason, `ReportsApi.removeUserAvatar`). Bewusst **nicht** umgesetzt: Kanalseite (existiert im Frontend nicht, Backend-Endpoint liegt bereit) und Avatare in Feed-/Suchkarten. **Client-seitige Größenprüfung (5 MB, `MAX_AVATAR_BYTES`) ist hier Pflicht, nicht nur UX:** Bei Requests weit über dem Multipart-Limit schließt Tomcat die Verbindung statt 413 zu antworten (hinter Caddy: 502) — der Wert ist nicht über `config.json` abrufbar und muss bei Backend-Änderungen von Hand nachgezogen werden. Die generierte Spec zeigt `setAvatar` wie schon `setThumbnail` fälschlich als `application/json` (Springdoc-Artefakt). `npm run generate:api` gegen das echte Backend hat dabei die von Hand ergänzten Einträge (Suche, `DELETING`) bestätigt. **Deploy-Reihenfolge: Backend zuerst taggen.** E2E `e2e/profile-photo.spec.ts` läuft nur auf Chromium, weil sie den geteilten Test-Account verändert und beide Engines parallel laufen würden. Nicht per E2E abgedeckt: der Admin-Button auf der Detailseite (Unit-Test für `ReportsApi`, Backend-Integrationstests für den Endpoint).
- [x] E2E-Login-Helper repariert (2026-09-19): `button[type="submit"]` traf seit der Header-Suchleiste (2026-09-13) zwei Elemente (Such-Submit + Login-Submit), `mobile-layout.spec.ts` (eingeloggt) und `upload.spec.ts` schlugen deshalb mit einer Strict-Mode-Violation fehl. Jetzt `page.locator('app-button').getByRole('button', { name: 'Log in' })`. Hinweis für lokale E2E-Läufe: das Backend limitiert Logins auf 5 pro 15 min und IP (In-Memory) — mehrere Suites hintereinander laufen schnell hinein ("Too many login attempts"); ein Backend-Neustart setzt die Buckets zurück.
